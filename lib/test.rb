...
class Calculator
  ...
  def calc(formula)
    ...
    #begin
      ...
      return [eval(value), units.flatten.join] unless @error[:unit_not_found].nil?
      [eval(value), calc_unit(units.flatten)]
    #rescue
  ...
  def si_unit(units)
    p unit = units.scan(/((?:#{reg(:metric)})?#{reg(:alter)})|(#{reg(:variable)})|((?:#{reg(:metric)})?(?:#{reg(:base)}|g){1}-*\d*)|(#{reg(:ari)})|(.)/)
  ...
  def calc_unit(units) #配列で単位を受け取る。
    ...
    par.reverse_each do |index|
      ...
      units[(index[0]+add)..(index[1]-add)] = unit + nil_back(units[(index[0]+add)..(index[1]-add)], unit)
    end
    units.compact!
    units.reject!{ |x| x =~ /\(|\)/ }
    serach_unit units
  ...
  def serach_unit(units)
    si_base_unit.merge(si_derived_unit).each do |key, value|
      break @alter = key if multi_div_unit(split_si_unit(value)[1]).sort == units.sort
    end
    p @alter
    @alter = si_alter_unit[@alter]
  end

  def nil_back(original, convert)
    array = []
    (original.size - convert.size).times { array << nil }
    array
  end
...
#private
  ...
  def si_base_unit
    @si_base_unit ||= YAML.load_file('si_base_unit.yml')
  end

  def si_derived_unit
    @si_derived_unit ||= YAML.load_file('si_derived_unit.yml')
  end

  def variable_unit
    @variable_unit ||= YAML.load_file('variable_unit.yml')
  end

  def metric_prefix_unit
    @metric_prefix ||= YAML.load_file('metric_prefix.yml')
  end

  def si_alter_unit
    @si_alter_unit ||= YAML.load_file('si_alter_unit.yml')
  end
